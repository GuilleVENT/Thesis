...
def get_X_and_y(pl_indx, pl_list):
	...
	save_npy(X_MIR, X_MIR_val, y_MIR, y_MIR_val,'MIR','RF',pl_indx,pl_list)
	save_npy(X_SPO, X_SPO_val, y_SPO, y_SPO_val,'SPO','RF',pl_indx,pl_list)
	save_npy(X_LYR, X_LYR_val, y_LYR, y_LYR_val,'LYR','RF',pl_indx,pl_list)
	save_npy(X_ALL, X_ALL_val, y_ALL, y_ALL_val,'ALL','RF',pl_indx,pl_list)
	...
	param_file = param_folder+'/MIR/'+'RF.pkl'
	if path.exists(param_file):
		print(' LOADING PARAMETERS:')
		print('	RF - Parameter Models')
		print('	MIR:')
		model_parameters_RF_MIR = open_parameters('RF','MIR',pl_indx)#,y_MIR,num_folds)
	else:
		print(' COMPUTING PARAMETERS:')
		print('	RF - Parameter Models')
		print('	MIR:')
		model_parameters_RF_MIR = perform_grid_search('RF',X_MIR,y_MIR,num_folds)
		save_parameters(param_folder,model_parameters_RF_MIR,'RF','MIR',pl_indx)		

	## RF - SPO
	param_file = param_folder+'/SPO/'+'RF.pkl'
	if path.exists(param_file):
		print(' LOADING PARAMETERS:')
		print('	RF - Parameter Models')
		print('	SPO:')
		model_parameters_RF_SPO = open_parameters('RF','SPO',pl_indx)#X_SPO,y_SPO,num_folds)
	else:
		print(' COMPUTING PARAMETERS:')
		print('	RF - Parameter Models')
		print('	SPO:')
		model_parameters_RF_SPO = perform_grid_search('RF',X_SPO,y_SPO,num_folds)
		save_parameters(param_folder,model_parameters_RF_SPO,'RF','SPO',pl_indx)			

	## RF - LYR
	param_file = param_folder+'/LYR/'+'RF.pkl'
	if path.exists(param_file):
		print(' LOADING PARAMETERS:')
		print('	RF - Parameter Models')
		print('	LYR:')
		model_parameters_RF_LYR = open_parameters('RF','LYR',pl_indx)#X_LYR,y_LYR,num_folds)
	else:
		print(' COMPUTING PARAMETERS:')
		print('	RF - Parameter Models')
		print('	LYR:')
		model_parameters_RF_LYR = perform_grid_search('RF',X_LYR,y_LYR,num_folds)
		save_parameters(param_folder,model_parameters_RF_LYR,'RF','LYR',pl_indx)			

	## RF - ALL 
	param_file = param_folder+'/ALL/'+'RF.pkl'
	if path.exists(param_file):
		print(' LOADING PARAMETERS:')
		print('	RF - Parameter Models')
		print('	ALL:')
		model_parameters_RF_ALL = open_parameters('RF','ALL',pl_indx)#X_ALL,y_ALL,num_folds)
	else:
		print(' COMPUTING PARAMETERS:')
		print('	RF - Parameter Models')
		print('	ALL:')
		model_parameters_RF_ALL = perform_grid_search('RF',X_ALL,y_ALL,num_folds)
		save_parameters(param_folder,model_parameters_RF_ALL,'RF','ALL',pl_indx)		
	'''else: ## 4 pkl saved with the parameters
		print(' # # # # # # ')
		print('- Loading all parameters saved:')
		model_parameters_RF_MIR = open_parameters('RF','MIR',pl_indx)
		model_parameters_RF_SPO = open_parameters('RF','SPO',pl_indx)
		model_parameters_RF_LYR = open_parameters('RF','LYR',pl_indx)
		model_parameters_RF_ALL = open_parameters('RF','ALL',pl_indx)'''
	...
	print_save(pl_indx, pl_list,'SPOTIFY DATASET',' RF ',y_true, y_pred, X_SPO_val, y_SPO_val, model_parameters_RF_SPO)

	## 		LYRICS DATASET 
	## 		RF
	y_true, y_pred = y_LYR_val, RF_clf_LYR.predict(X_LYR_val)
	print_save(pl_indx,pl_list,'LYRICS DATASET',' RF ',y_true, y_pred, X_LYR_val, y_LYR_val, model_parameters_RF_LYR)

	## 		MIR DATASET
	## 		RF
	y_true, y_pred = y_MIR_val, RF_clf_MIR.predict(X_MIR_val)
	print_save(pl_indx,pl_list,'  MIR  DATASET',' RF ',y_true, y_pred, X_MIR_val, y_MIR_val, model_parameters_RF_MIR)

	## 		ALL DATASET 
	##		RF
	y_true, y_pred = y_ALL_val, RF_clf_ALL.predict(X_ALL_val)
	print_save(pl_indx,pl_list,'  ALL  DATASET',' RF ',y_true, y_pred, X_ALL_val, y_ALL_val, model_parameters_RF_ALL)
...
def perform_grid_search(model, X, y, num_folds):
	...
	if model == "RF":
		...
		param_grid = {'bootstrap': [True],
					...
					'min_samples_leaf': [3, 4, 5],
					'min_samples_split': [8, 10, 12],
					'n_estimators': [100,1000] ## leaving this one out for now 
					}
		##
		##
